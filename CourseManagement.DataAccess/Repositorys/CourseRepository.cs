...
namespace CourseManagement.DataAccess.Repositorys {
    public class CourseRepository : ICourseRepository {
        ...
        public async Task EnrollCourse(EnrollCourseRequest request) {
            ...
            if (courseFounded.CourseType == CourseType.ProCourse && !HasActivePremium(userFound)) {
                throw new ArgumentException("You must have an active VIP account to learn this course");
            }
        ...
        public async Task<LearnCourseResponse> Learn(LearnCourseRequest request, string userEmail) {
            ...
            if (course.CourseType == CourseType.ProCourse && !HasActivePremium(userFound)) {
                throw new ArgumentException("You must have an active VIP account to learn this course");
            }
        ...
        private static bool HasActivePremium(AppUser user) {
            return user.VipStatus == VipStatus.Premium
                && user.VipExpirationDate.HasValue
                && user.VipExpirationDate.Value > DateTime.Now;
        }
    }
}
